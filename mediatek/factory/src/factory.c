...
static item_t ftm_menu_items[] = {
    ...
    //item(ITEM_CLREMMC,  uistr_clear_emmc),  //remove CLREMMC menu
#endif
...
static item_t ftm_test_items[] = {
    ...
    item(ITEM_CALIBRATION_STATUS,uistr_calibration_status),
#endif
...
//#endif
  ...
  //  item(ITEM_LOOPBACK_PHONEMICSPK, uistr_info_audio_loopback_phone_mic_speaker),
   ...
   // item(ITEM_LOOPBACK, uistr_info_audio_loopback),
    ...
    item(ITEM_WAVEPLAYBACK, uistr_info_audio_ringtone),
  #endif
    ...
    item(ITEM_PSCALI,uistr_ps_calic),
#endif
...
static void print_verinfo(char *info, int *len, char *tag, char *msg)
{
	...
	int max_len = gr_fb_width() / CHAR_WIDTH; // xiangfei.peng 20140504 for version display abnormal
    ...
    while(msg_len>0) {
		int newline_len = 0;
		bool newline_exist = false;
        buf_len = max_len - 4;
        buf_len = (msg_len > buf_len ? buf_len : msg_len);
		//add a process to search "\n" start xiangfei.peng
		while(newline_len < buf_len)
		{
			if(msg[newline_len] == '\n')
			{
				newline_exist = true;
				buf_len = newline_len + 1; //char account before and with '\n'.
				break;
			}
			newline_len ++;
		}
		...
		if(newline_exist == false) // modified 20140504 xiangfei.peng
		{	//ignore twice '\n' if msg has '\n' in string.
        		_len += sprintf(info + _len, "\n");
		}
		// common part start
...
if(is_support_modem(5)){
	...
	// 增加前后摄像头和lcm的识别显示 苏 勇 2012年09月10日 11:07:31
	{
		int fd=-1;
		char cameraName[128]  = "unknown\nunknown";
		char lcmName[128]="unknown";
		unsigned char i=0,j=0;
		camera_open_once();
		fd = open("/sys/devices/platform/image_sensor/camera_name",O_RDONLY);
	    if(fd!=-1) 
		{
	        read(fd,cameraName,128);
	        if(cameraName[strlen(cameraName)-1]=='\n') cameraName[strlen(cameraName)-1]=0;
			close(fd);
	    }
		print_verinfo(info, &len,  "Camera info ", cameraName);
		...
		fd = open("/sys/devices/platform/mtkfb.0/lcm_name",O_RDONLY);
	    if(fd!=-1) 
		{
			read(fd,lcmName,128);
	        if(lcmName[strlen(lcmName)-1]=='\n') lcmName[strlen(lcmName)-1]=0;
			close(fd);
	    }
		print_verinfo(info, &len,  "Lcm info    ", lcmName);
	}
// 苏 勇 2013年09月13日 16:14:04    property_get("ro.build.date", val, "TBD");
// 苏 勇 2013年09月13日 16:14:04    print_verinfo(info, &len,  "Build Time  ", val);
...
// 苏 勇 2013年09月13日 16:13:36    print_verinfo(info, &len,  "Kernel Ver. ", ptr);
// 苏 勇 2013年09月13日 16:13:36    property_get("ro.build.version.release", val, "unknown");
...
// 苏 勇 2013年09月13日 16:13:46	property_get("ro.custom.build.version",val,"unknown");
// 苏 勇 2013年09月13日 16:13:46	print_verinfo(info, &len,  "Custom Build Verno.     ", val);
...
char ** trans_verinfo(const char *str, int *line)
{
    ...
    pstrs = (char**)malloc(max * sizeof(char*));
    //
    if (!pstrs) {
...
int main(int argc, char **argv)
{
    ...
    //add for saving test report
	item_t rpt_items[ITEM_MAX_IDS + 1];
	int chosen_item_temp;//added by sen.luo 20130708
    text_t vi_title;
    text_t ate_title;
    text_t rbtn;
    text_t info;
    ...
    else if(FACTORY_BOOT == bootMode)
    {
		...
		while (!exit) 
		{
			int chosen_item = fiv.run(&fiv, NULL);
			 chosen_item_temp = chosen_item;//added by sen.luo 20130708
			switch (chosen_item) 
			{
			case ITEM_MUI_TEST:
				quit = false;
	...
	//end
		...
		if (buf)
			free(buf);
	//added by sen.luo 20130708
	if(chosen_item_temp==ITEM_POWEROFF)
	{
	ui_printf("PowerOff...\n");
	sync();
	reboot(RB_POWER_OFF);
