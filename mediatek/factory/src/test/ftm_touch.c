...
enum {
    ...
    ITEM_TOUCH_ZOOM,
    ITEM_TOUCH_FREEMODE,
    ITEM_TOUCH_GETTPINFO, /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
    ITEM_TOUCH_PASS,
    ITEM_TOUCH_FAIL,
    ITEM_TOUCH_CHOOSER,
};
...
item_t ctype_touch_items[] = {
    ...
    item(ITEM_TOUCH_FREEMODE,    uistr_info_touch_freemode),
    item(ITEM_TOUCH_GETTPINFO,    uistr_info_touch_get_tpinfo), /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
    item(ITEM_TOUCH_PASS,        uistr_pass),
    item(ITEM_TOUCH_FAIL,        uistr_fail),
    item(-1            ,  NULL),
};
...
struct touch {
    ...
    text_t title;
    text_t text;     /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
    char  info[256]; /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
};
...
#define TOUCH_UPDATE_FR_DIV 4 // 2 //ckt guoyi modify 2014-5-12
#endif
...
void get_tp_info(struct touch *priv) /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
{
    struct touch *tpd = (struct touch *)priv;
    struct itemview *iv = &(tpd->menu); 
    int fd=-1;
    char tpName[128]="unKnown";
    fd = open("/sys/devices/platform/mtk-tpd/chipinfo",O_RDONLY);
    if(fd!=-1) 
    {
        read(fd,tpName,128);
    ...
    snprintf(tpd->info, sizeof(tpd->info), "Tp info: \n%s \n", tpName);
    iv->set_text(iv, &(tpd->text));
    iv->redraw(iv);
}
...
int touch_entry(struct ftm_param *param, void *priv) {
    ...
    init_text(&(tpd->text), &tpd->info[0], COLOR_YELLOW); /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
    ...
    do {
        ...
        case ITEM_TOUCH_CHOOSER:
            ...
            scene_init(tpd);
            switch (tpd->scene) {
            case ITEM_TOUCH_GETTPINFO:
                get_tp_info(tpd); /*ckt guoyi add for ckt base , get tp info in factorymode 2014-5-7*/
                break;
