...
static void *rtc_tm_pthread(void *priv)
{
	...
	rtc_init_tm(&tm, 2013, 12, 31, 23, 59, 57);
	...
	while (1) {
		...
		if(alm_flag==0 && tm.tm_hour==0 && tm.tm_min==0 && tm.tm_sec==0){//ckt helin 20131203
			LOGD(TAG "pthread_create:rtc_alm_pthread\n");
			alm_flag=1;
			pthread_create(&rtc->alm_pt, NULL, rtc_alm_pthread, priv);
		}
		else if(tm.tm_hour==0 && tm.tm_min==0 && tm.tm_sec==3){
			goto ret;
		}
		...
		if (tm.tm_sec == 0 && tm.tm_min == 0 && tm.tm_hour == 0 &&
		    tm.tm_mday == 1 && tm.tm_mon == 0 && tm.tm_year == 114) {
			break;
		}
#else
...
//	pthread_create(&rtc->alm_pt, NULL, rtc_alm_pthread, priv);
	...
	alm.enabled = 0;
	rtc_init_tm(&alm.time, 2013, 1, 1, 0, 0, 0);
	r = ioctl(rtc->fd, RTC_WKALM_SET, &alm);
	if (r < 0)
