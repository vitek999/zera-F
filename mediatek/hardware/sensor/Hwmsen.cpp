      ...
      mHasPendingEvent(false)
{
...
#if defined (CKT_USER_INEMOENGINE)  // 苏 勇 2014年06月06日 16:51:31
	mPendingEvents[rotationvector].version = sizeof(sensors_event_t);
    mPendingEvents[rotationvector].sensor = ID_ROTATION_VECTOR;
	...
	mPendingEvents[lineraacc].version = sizeof(sensors_event_t);
    mPendingEvents[lineraacc].sensor = ID_LINEAR_ACCELERATION;
    mPendingEvents[lineraacc].type = SENSOR_TYPE_LINEAR_ACCELERATION;
...
int Hwmsen::enable(int32_t handle, int en) {
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		...
		handle != ID_PROXIMITY && handle != ID_LIGHT &&handle !=ID_PRESSURE
#if defined (CKT_USER_INEMOENGINE)  // 苏 勇 2014年06月06日 16:52:06
		&&handle != ID_ROTATION_VECTOR && handle != ID_GRAVITY &&handle !=ID_LINEAR_ACCELERATION
#endif /* CKT_USER_INEMOENGINE */
	   )
	{
...
void Hwmsen::processEvent(int code, int value)
{
	...
	for(i =0; i < MAX_ANDROID_SENSOR_NUM; i++)
	{
		...
		case ID_LIGHT:
			 ...
			 mPendingEvents[light].light = (float)sensors_data.data[ID_LIGHT].values[0]/sensors_data.data[ID_LIGHT].value_divide;
		...
		case ID_PRESSURE:
			 ...
			 mPendingEvents[pressure].type = SENSOR_TYPE_PRESSURE;
			 mPendingEvents[pressure].sensor = sensors_data.data[ID_PRESSURE].sensor;
			 mPendingEvents[pressure].pressure = (float)sensors_data.data[ID_PRESSURE].values[0]/sensors_data.data[ID_PRESSURE].value_divide;
			 mPendingEvents[pressure].timestamp = sensors_data.data[ID_PRESSURE].time;
			 break;



			 
#if defined (CKT_USER_INEMOENGINE)  // 苏 勇 2014年06月06日 16:52:44
		case ID_ROTATION_VECTOR:
			ALOGE("hwmsen: rotationvector %d ",sensors_data.data[ID_ROTATION_VECTOR].values[0]);
			mPendingMask |= 1<<rotationvector;
			mPendingEvents[rotationvector].type = SENSOR_TYPE_ROTATION_VECTOR;
			mPendingEvents[rotationvector].sensor = sensors_data.data[ID_ROTATION_VECTOR].sensor;
			mPendingEvents[rotationvector].data[4] = sensors_data.data[ID_ROTATION_VECTOR].status;
			mPendingEvents[rotationvector].data[0] = (float)sensors_data.data[ID_ROTATION_VECTOR].values[0];
			mPendingEvents[rotationvector].data[1] = (float)sensors_data.data[ID_ROTATION_VECTOR].values[1];
			mPendingEvents[rotationvector].data[2] = (float)sensors_data.data[ID_ROTATION_VECTOR].values[2];

			mPendingEvents[rotationvector].data[0]/=sensors_data.data[ID_ROTATION_VECTOR].value_divide;
			mPendingEvents[rotationvector].data[1]/=sensors_data.data[ID_ROTATION_VECTOR].value_divide;
			mPendingEvents[rotationvector].data[2]/=sensors_data.data[ID_ROTATION_VECTOR].value_divide;
			mPendingEvents[rotationvector].timestamp = sensors_data.data[ID_ROTATION_VECTOR].time;
			break;
			
		case ID_GRAVITY:
			mPendingMask |= 1<<gravity;
			mPendingEvents[gravity].type = SENSOR_TYPE_GRAVITY;
			mPendingEvents[gravity].sensor = sensors_data.data[ID_GRAVITY].sensor;
			mPendingEvents[gravity].data[4] = sensors_data.data[ID_GRAVITY].status;
			mPendingEvents[gravity].data[0] = (float)sensors_data.data[ID_GRAVITY].values[0];
			mPendingEvents[gravity].data[1] = (float)sensors_data.data[ID_GRAVITY].values[1];
			mPendingEvents[gravity].data[2] = (float)sensors_data.data[ID_GRAVITY].values[2];

			mPendingEvents[gravity].data[0]/=sensors_data.data[ID_GRAVITY].value_divide;
			mPendingEvents[gravity].data[1]/=sensors_data.data[ID_GRAVITY].value_divide;
			mPendingEvents[gravity].data[2]/=sensors_data.data[ID_GRAVITY].value_divide;
			mPendingEvents[gravity].timestamp = sensors_data.data[ID_GRAVITY].time;
			break;
			
		case ID_LINEAR_ACCELERATION:
			mPendingMask |= 1<<lineraacc;
			mPendingEvents[lineraacc].type = SENSOR_TYPE_LINEAR_ACCELERATION;
			mPendingEvents[lineraacc].sensor = sensors_data.data[ID_LINEAR_ACCELERATION].sensor;
			mPendingEvents[lineraacc].data[4] = sensors_data.data[ID_LINEAR_ACCELERATION].status;
			mPendingEvents[lineraacc].data[0] = (float)sensors_data.data[ID_LINEAR_ACCELERATION].values[0];
			mPendingEvents[lineraacc].data[1] = (float)sensors_data.data[ID_LINEAR_ACCELERATION].values[1];
			mPendingEvents[lineraacc].data[2] = (float)sensors_data.data[ID_LINEAR_ACCELERATION].values[2];

			mPendingEvents[lineraacc].data[0]/=sensors_data.data[ID_LINEAR_ACCELERATION].value_divide;
			mPendingEvents[lineraacc].data[1]/=sensors_data.data[ID_LINEAR_ACCELERATION].value_divide;
			mPendingEvents[lineraacc].data[2]/=sensors_data.data[ID_LINEAR_ACCELERATION].value_divide;
			mPendingEvents[lineraacc].timestamp = sensors_data.data[ID_LINEAR_ACCELERATION].time;
			break;
...
int Hwmsen::setDelay(int32_t handle, int64_t ns)
{
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		handle != ID_ORIENTATION && handle != ID_GYROSCOPE && 
		handle != ID_LIGHT && handle != ID_PRESSURE
#if defined (CKT_USER_INEMOENGINE)  // 苏 勇 2014年06月06日 16:53:11
		&&handle != ID_ROTATION_VECTOR && handle != ID_GRAVITY &&handle !=ID_LINEAR_ACCELERATION
#endif /* CKT_USER_INEMOENGINE */
	  )
	{
    ...
    what = handle;
    if (uint32_t(what) >= numSensors) // 注意这里是id/handle和 numSensors中的枚举进行了比较,这是2种不同的东西,按说不可以比较,但这里可以保证mDelays的数组不越界 苏 勇 2014年05月30日 15:05:10
        return -EINVAL;
