...
#define KXTJ2_1009_DEV_NAME        "KXCJK_1013"
/*----------------------------------------------------------------------------*/
...
static int KXTJ2_1009_init_flag =0; // 0<==>OK -1 <==> fail
...
static void KXTJ2_1009_power(struct acc_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "KXCJK_1013"))
			{
		...
		{
			if (!hwPowerDown(hw->power_id, "KXCJK_1013"))
			{
...
static int KXTJ2_1009_CheckDeviceID(struct i2c_client *client)
{
	...
	if(false)
	{
		printk("KXCJK_1013_CheckDeviceID 0x%x failt!\n ", databuf[0]);
		return KXTJ2_1009_ERR_IDENTIFICATION;
	}
	else
	{
		printk("KXCJK_1013_CheckDeviceID 0x%x pass!\n ", databuf[0]);
	}
...
static int KXTJ2_1009_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	{
		printk("KXCJK_1013 read Dataformat failt \n");
		return KXTJ2_1009_ERR_I2C;
	}
	...
	printk("KXCJK_1013_SetDataFormat OK! \n");
...
static int KXTJ2_1009_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	{
		printk("KXCJK_1013 read rate failt \n");
		return KXTJ2_1009_ERR_I2C;
	}
	...
	KXTJ2_1009_SetPowerMode(client, cur_sensor_power/*true*/);
	printk("KXCJK_1013_SetBWRate OK! \n");
...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	printk("KXCJK_1013_init_client OK!\n");
#ifdef CONFIG_KXTJ2_1009_LOWPASS
...
static int KXTJ2_1009_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	{
		sprintf(buf, "KXCJK_1013_ReadRawData %04x %04x %04x", obj->data[KXTJ2_1009_AXIS_X], 
...
static int KXTJ2_1009_JudgeTestResult(struct i2c_client *client, s32 prv[KXTJ2_1009_AXES_NUM], s32 nxt[KXTJ2_1009_AXES_NUM])
{
    ...
    if ( test_result != 0xaa ) 
	{
        GSE_ERR("KXCJK_1013_JudgeTestResult failt\n");
        res = -EINVAL;
    }
    return res;
}
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = kxcjk_1013_get_cust_acc_hw();
...
{
	struct acc_hw *hw = kxcjk_1013_get_cust_acc_hw();
	GSE_FUN();
...
#define CKT_G_AND_M_SENSOR_DIRECTION_DEBUG
 ...
 static int  lsm303d_set_acc_direction(struct i2c_client *client,int direction)
 {
 int err = -1;
struct kxtj2_1009_i2c_data *obj = i2c_get_clientdata(client);
if(err = hwmsen_get_convert(obj->hw->direction, &obj->cvt))
...
i2c_set_clientdata(kxtj2_1009_i2c_client, obj); 
}
 static ssize_t gsensor_test(struct kobject *kobj,
			struct bin_attribute *attr,
			char *buf, loff_t off, size_t count)
{
		uint16_t val;
		struct kxtj2_1009_i2c_data *gsensor_i2c_data = i2c_get_clientdata(kxtj2_1009_i2c_client);
		...
		{
		 lsm303d_set_acc_direction(kxtj2_1009_i2c_client,gsensor_i2c_data->hw->direction);
		printk( " luosen write gsensor direction = %d \n",gsensor_i2c_data->hw->direction);
	}
	return count;
}
static ssize_t gsensor_read(struct kobject *kobj,
			struct bin_attribute *attr, 
			char *buf, loff_t off, size_t count)
{
	printk("gsensor_read!!!!!\n");
...
static struct bin_attribute gsensor_mode_attr = {
	.attr = {
		.name = "gsensor_test",
...
static int __init kxtj2_1009_init(void)
{
	...
	#if defined(CKT_G_AND_M_SENSOR_DIRECTION_DEBUG)	 
	 int ret;
	ret = sysfs_create_bin_file(&(module_kset->kobj), &gsensor_mode_attr);
	if (ret) {
		printk(KERN_ERR "<gsensor> Failed to create sys file\n");
		return -ENOMEM;
	}
       #endif

	//hwmsen_gsensor_add(&KXTJ2_1009_init_info);
	i2c_register_board_info(0, &i2c_kxtj2_1009, 1);
