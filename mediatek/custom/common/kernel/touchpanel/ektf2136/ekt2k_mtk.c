...
#include <linux/hwmsensor.h>hwmsensor.h
#include <linux/hwmsen_dev.h>
#include <linux/sensors_io.h>
static int PROXIMITY_STATE = 0;
static  int elan_ps_get_state(void);
#endif
...
static int X_RESOLUTION=480; //0x00;  
static int Y_RESOLUTION=800; //0x00;
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	int rc = 0;
	 if(buf[0] == 0xfa){
		...
		printk("elan ps data, %02x %02x %02x %02x", buf[0],  buf[1],buf[2], buf[3]);
		sensor_data.values[0] = elan_ps_get_state();
...
static int elan_ts_send_cmd(struct i2c_client *client, uint8_t *cmd, size_t size)
{
	...
	if (elan_i2c_send_data(client, cmd, size) != size) {
		...
		printk("[elan] elan_ts_send_cmd ok");
	}
	return size;
}

static int elan_ps_mod_switch(int onoff_state)
{
	int ret = 0;
	uint8_t cmd[4] ={0x54, 0xC1, 0x00, 0x01};
	...
	printk("elan_ps_mod_switch  onoff_state %d\n",onoff_state);
	ret = elan_ts_send_cmd(private_ts->client, cmd, 4);
...
static int elan_ps_get_state(void)
{
	printk("elan_ps_get_state PROXIMITY_STATE %d\n",PROXIMITY_STATE);
	return PROXIMITY_STATE;
}

s32 elan_ts_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
                   void *buff_out, s32 size_out, s32 *actualout)
{
	...
	switch (command){
		case SENSOR_DELAY:
			printk("elan_ts_ps_operate SENSOR_DELAY\n");
			if ((buff_in == NULL) || (size_in < sizeof(int))){
				printk("[elan error] Set delay parameter error!");
				err = -EINVAL;
			}
			break;

		case SENSOR_ENABLE:
			printk("elan_ts_ps_operate SENSOR_ENABLE");
			if ((buff_in == NULL) || (size_in < sizeof(int))){
				printk("[elan error] Enable sensor parameter error!");
				err = -EINVAL;
			}
			else{
				value = *(int *)buff_in;
				elan_ps_mod_switch(value);
			}
			break;

		case SENSOR_GET_DATA:
			printk("elan_ts_ps_operate SENSOR_GET_DATA\n");
			if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))){
				printk("[elan error] Get sensor data parameter error!");
				err = -EINVAL;
			}
			else{
		...
		default:
			printk("[elan] proxmy sensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
	return err;
...
//for firmware auto-upgrade            
	...
	obj_ps.sensor_operate = elan_ts_ps_operate;

	if ((err = hwmsen_attach(ID_PROXIMITY, &obj_ps))){
		printk("[elan error] hwmsen attach fail, return:%d.", err);
	}
#endif
...
static ssize_t show_chipinfo(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	switch (FW_ID)
	{
		case 0x14FD: // 创世 苏 勇 2013年11月13日 09:48:12
			...
			break;
		case 0x27e6: // 华瑞川sen.luo
			return sprintf(buf,"ID:0x%x VER:0x%x IC:ektf2136 VENDOR:hrc\n",FW_ID, FW_VERSION);	
			break;
		default:
